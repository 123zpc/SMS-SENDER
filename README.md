...
```text
goAsync() + 专用后台线程 + 同步 OkHttp execute() + 短时 PARTIAL_WAKE_LOCK
```

这样做的目的，是让系统通过短信广播拉起应用后，在广播异步生命周期内尽快完成这一次 HTTP 请求。它不做保活、不做队列、不做重试，也不启动前台服务。

如果仍然只有打开应用界面时才能转发，通常说明系统没有在后台投递 `SMS_RECEIVED`，或厂商系统禁止该应用后台联网/自启动。此时需要在系统设置中允许后台运行、后台联网、自启动，并关闭电池优化。
...
- `0.4.1`：修复短信后台态转发链路，使用短时 WakeLock 和专用线程完成广播内 HTTP 请求。
