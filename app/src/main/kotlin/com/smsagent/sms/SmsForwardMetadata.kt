package com.smsagent.sms

import com.smsagent.util.TimeFormatter

data class SmsForwardMetadata(
    val traceId: Long,
    val receivedAtMillis: Long,
    val pid: Int,
    val processName: String,
) {
    fun logLine(httpResult: String, durationMillis: Long): String {
        return "traceId=${formatTraceId(traceId)} " +
            "time=${TimeFormatter.format(System.currentTimeMillis())} " +
            "receivedAt=${TimeFormatter.format(receivedAtMillis)} " +
            "pid=$pid " +
            "processName=$processName " +
            "threadName=${Thread.currentThread().name} " +
            "httpResult=$httpResult " +
            "durationMs=$durationMillis"
    }

    private fun formatTraceId(value: Long): String {
        return "T" + value.toString().padStart(5, '0')
    }
}
